...
"""
        ...
        # passed = out.endswith("<<HUMANEVAL_TEST_RESULT>>: PASS")
...
def process_prompt(task, seeds, mode, output_path,code_agent=None):
    ...
    for seed in seeds:
        ...
        for _ in range(10):  # N_SAMPLES
            ...
            # p_prompt = add_entrypoint_constraint(prompt, entry_point)
            p_mut_prompt = add_entrypoint_constraint(rad_mut, entry_point)

            # base = loop.run_until_complete(generate_code(p_prompt, code_agent))
        ...
        append_mutation_codes_csv(
            path="results/generated_codes.csv",
            task_id=task_id,
...
def process_code_mutation(task, seeds, mode, output_path,code_agent=None):
    ...
    for seed in seeds:
        ...
        for _ in range(10):  # N_SAMPLES
            ...
            # mut = loop.run_until_complete(generate_code_sys_prompt(mutant, code_agent))
            # p_prompt = add_entrypoint_constraint(prompt, entry_point)
